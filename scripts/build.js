...
function getDeploymentDomain() {
  // Use EXPO_PUBLIC_DOMAIN env var if set, otherwise default to Railway production domain
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    if (manifest.extra.expoClient.iconUrl) {
      manifest.extra.expoClient.iconUrl = `${baseUrl}/assets/images/offres365.png`;
    }
    if (manifest.extra.expoClient.android?.adaptiveIcon?.foregroundImageUrl) {
      manifest.extra.expoClient.android.adaptiveIcon.foregroundImageUrl = `${baseUrl}/assets/images/offres365-adaptive.png`;
    }
